...
// `useParamsCompat` falls back to `{}` without a real
// `<RouterProvider>` (see `routerCompat.ts`). The widget reads
// `boardId` from params on mount; mock the helper so the test fixture
// can pin the value without spinning up the full production router.
...
function renderSettings(boardId = "brd-1"): {
  ...
  const tree: ReactElement = (
    <QueryClientProvider client={client}>
      <ActiveSpaceProvider>
        <ToastProvider>
          <TestRouter path={`/boards/${boardId}/settings`}>
            <BoardSettings />
          </TestRouter>
        </ToastProvider>
      </ActiveSpaceProvider>
