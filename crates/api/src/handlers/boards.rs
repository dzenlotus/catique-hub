...
/// # Errors
///
/// Forwards every error from `BoardsUseCase::list_recent_boards`.
#[tauri::command]
pub async fn list_recent_boards(state: State<'_, AppState>) -> Result<Vec<Board>, AppError> {
    BoardsUseCase::new(&state.pool).list_recent_boards()
}

/// IPC: record a board visit. Fire-and-forget on board-open in the UI.
///
/// # Errors
///
/// Forwards every error from `BoardsUseCase::track_board_visit`.
#[tauri::command]
pub async fn track_board_visit(
    state: State<'_, AppState>,
    board_id: String,
) -> Result<(), AppError> {
    BoardsUseCase::new(&state.pool).track_board_visit(&board_id)
}

/// IPC: wipe every row from `recent_boards`. Backs the AppSidebar's
/// "Clear" affordance — explicit user intent. Pinned boards are
/// unaffected (sibling table). Idempotent: clearing an already-empty
/// table is a silent no-op.
///
/// # Errors
///
/// Forwards every error from `BoardsUseCase::clear_recent_boards`.
#[tauri::command]
pub async fn clear_recent_boards(state: State<'_, AppState>) -> Result<(), AppError> {
    BoardsUseCase::new(&state.pool).clear_recent_boards()
}
